...
func TestParseQuote_PrintStaticValues(t *testing.T) {
	...
	for _, f := range files {
		...
		t.Run(f, func(t *testing.T) {
			...
			fmspc, err := extractFMSPC(q)
			if err != nil {
				t.Logf("extractFMSPC error: %v", err)
			}
			t.Logf("File:      %s", f)
...
func TestVerify_Integration(t *testing.T) {
	...
	for _, tc := range quoteTestCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			result, err := client.Verify(q)
			if err != nil {
				t.Fatalf("Verify: %v", err)
			}

			t.Logf("TCBStatus=%s AdvisoryIDs=%v", result.Status, result.AdvisoryIDs)

			if result.Status != tc.wantTCBStatus {
				t.Errorf(
					"TCBStatus = %q, want %q\n"+
						"NOTE: Intel may have issued a TCB update since 2026-02-20.\n"+
						"If the new status is expected, update wantTCBStatus in quoteTestCases.",
					result.Status, tc.wantTCBStatus,
				)
			}
		})
	}
